...
class MTCCMBertForMMTokenClassificationCRF(BertPreTrainedModel):
    ...
    """
    def __init__(self, config, use_roberta=True, layer_num1=1, layer_num2=1, layer_num3=1,  num_labels=2, auxnum_labels=2, dropout_rate=0.2):
        super(MTCCMBertForMMTokenClassificationCRF, self).__init__(config)
        self.num_labels = num_labels
        self.auxnum_labels = auxnum_labels
        self.use_roberta = use_roberta
        if use_roberta:
            robert_config = RobertaConfig.from_json_file('/mnt/nfs-storage-titan/yoli_projects/roberta-base-cased/config.json')  # changed to large or base
            self.roberta = RobertaModel(robert_config)
        else:
            self.bert = BertModel(config)
        ...
        self.self_image_attention = BertSelfEncoder(config)
        self.img_dropout = nn.Dropout(dropout_rate)
        self.img_classifier = nn.Linear(config.hidden_size, 2089)
        self.klloss = nn.KLDivLoss(reduction= 'batchmean')
        self.vismap2text_v2 = nn.Linear(2048, config.hidden_size)
        self.txt2img_attention = BertCoAttention(config)
        self.LayerNorm = BertLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.intermediate = BertIntermediate(config)
        self.output = BertOutput(config)
        self.LayerNorm_top = BertLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.classifier = nn.Linear(config.hidden_size, num_labels)
    ...
    def forward(self, input_ids, segment_ids, input_mask, added_attention_mask, visual_embeds_att, trans_matrix,imagelabel,alpha, beta,
                labels=None, auxlabels=None):
        batch_size, lenth = input_ids.size()
        if self.use_roberta:
            roberta_output = self.roberta(input_ids, input_mask)
            sequence_output = roberta_output[0]
        else:
            sequence_output, _ = self.bert(input_ids, token_type_ids=segment_ids,
        ...
        main_addon_sequence_encoder = self.self_attention(sequence_output, extended_txt_mask)
        main_addon_sequence_output = main_addon_sequence_encoder[-1]
        aux_bert_feats = self.aux_classifier(main_addon_sequence_output)
        aux_output = nn.Softmax(dim=-1)(aux_bert_feats)
        ...
        img_sequence_output = self.self_image_attention(converted_vis_embed_map,extended_img_mask )
        img_sequence_output = img_sequence_output[-1]
        img_sequence_output_mean = img_sequence_output.mean(dim= 1)
        img_sequence_output_dropout = self.img_dropout(img_sequence_output_mean)
        img_tag = self.img_classifier(img_sequence_output_dropout)
        img_tag_softmax = F.softmax(img_tag, dim= -1)
        img_tag_loss = self.klloss(img_tag_softmax.log(),imagelabel.cuda())
        cross_output_layer = self.txt2img_attention(main_addon_sequence_output, img_sequence_output, extended_img_mask)
        c = torch.sum(aux_output * aux_output, dim=-1)
        gate_value = c.unsqueeze(-1)
        reverse_gate_value = torch.neg(gate_value).add(1)
        gated_converted_att_vis_embed = torch.mul(reverse_gate_value, cross_output_layer)
        gated_main_addon_sequence_output = torch.mul(gate_value, main_addon_sequence_output)
        weighted_add_output = torch.add(gated_main_addon_sequence_output, gated_converted_att_vis_embed)
        weighted_add_norm_output = self.LayerNorm(weighted_add_output)
        ...
        final_output = self.LayerNorm_top(layer_output+main_addon_sequence_output)
        final_bert_feats = self.classifier(final_output)
        if labels is not None:
            loss_fct = CrossEntropyLoss()
            active_loss = input_mask.view(-1) == 1
            active_logits = aux_bert_feats.view(-1, self.auxnum_labels)
            active_labels = torch.where(
                active_loss, auxlabels.view(-1), torch.tensor(loss_fct.ignore_index).type_as(auxlabels)
            )
            aux_loss = loss_fct(active_logits, active_labels)

            main_loss = - self.crf(final_bert_feats, labels, mask=input_mask.byte(), reduction='mean')
            loss = main_loss + alpha* img_tag_loss + beta*aux_loss
            return loss
        else:
